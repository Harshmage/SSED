...
Function Get-NexusMods {
	...
	Try {
		...
		$global:dlResponse = (Invoke-WebRequest "https://api.nexusmods.com/v1/games/$nexusgameID/mods/$nexusmodID/files/$latestfileid/download_link.json" -Headers $nexusHeaders).Content | ConvertFrom-Json | Where-Object { $_.short_name -eq "Nexus CDN" } | Select-Object -ExpandProperty URI
		If ($SEGame -eq "FOSE") {
...
If (!(Test-Path $env:ProgramFiles\7-Zip\7z.exe)) {
	Write-Error -Message "7-Zip x64 Path Not Found!"
...
If (!($halt)) {
	...
	If ($dl.ver -gt $currentSE) {
		...
		If (!(Test-Path $gamepath\$subfolder)) {
			Write-Log -Message "Subfolder $($gamepath)\$($subfolder) does not exist, looking for possible subfolder" -Level Info
			$subfolder = Get-ChildItem -Path $gamepath -Directory | Where-Object { $_.Name -match "$($SEGame)" } | Select-Object -First 1 | Select-Object -ExpandProperty Name
		}
		Write-Log -Message "Subfolder found: $($gamepath)\$($subfolder)" -Level Info
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA3SvJPy1TE76tJ
# OBlFRs7AU7Lc+FqiqoI91GvUZtK/P6CCB0IwggN5MIIC/qADAgECAhAcz51nzeIZ
...
# AQsxDjAMBgorBgEEAYI3AgEVMC8GCSqGSIb3DQEJBDEiBCAT9xjj0d0E/z4KPZq/
# xGh7YBLJvyhsDvNfP+wxn+xoazALBgcqhkjOPQIBBQAEZzBlAjEAyVZQ4h4Muu1M
# e9SkCRqHXbSRKC9VFsBvnGw3GdbBfB/zV1qW5tewz3DTBa6FiYPcAjBPda8JYb89
# Ogp4sTbARZAtRTOFkNE9qqb97fAjIg/WXhyztvgyEaOOSRmtSGe9kYihgg8WMIIP
...
# MDEwDQYJYIZIAWUDBAIBBQAEIF/UWzYASi5MJiZVNR/DOs/z6mQI6RQ3V8gENk1N
# SBh7AghAVJOYUK1FVhgPMjAyNTA0MjcwNjI0NTFaMAMCAQGgggwAMIIE/DCCAuSg
...
# hvcNAQkFMQ8XDTI1MDQyNzA2MjQ1MVowKAYJKoZIhvcNAQk0MRswGTALBglghkgB
# ZQMEAgGhCgYIKoZIzj0EAwIwLwYJKoZIhvcNAQkEMSIEILeIaye30oAEZNdwc5HI
# OCs9noKgWxzmeKAMm/rrzgrGMIHJBgsqhkiG9w0BCRACLzGBuTCBtjCBszCBsAQg
...
# FRh/IJCa7B7pC4ueLxm/xMaM12C38bBsLRhDTare8RECIQDZF6eHSA7rChbOkr+c
# XhiQVkN9AoID9ycCRxJ9WsJWPw==
